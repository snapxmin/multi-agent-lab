...
export default function TopologyCanvas({ scenario, color, t, extraPackets = [], compact = false, ctxBarFor = [], onNodeClick, flashNode }: Props) {
  ...
  const visibleNode = (n: NodeDef) => n.appearAt === undefined || t >= n.appearAt - 0.35
  const nodeScale = (n: NodeDef) => {
    if (n.appearAt === undefined) return 1
  ...
  return (
    <svg viewBox="0 0 860 560" className="h-full w-full" style={{ display: 'block' }}>
      ...
      {scenario.edges.map((e) => {
        ...
        const c = curveFor(a, b, e.from + e.to)
        return (
          <path
            key={`${e.from}-${e.to}`}
            ...
            strokeOpacity={e.faint ? 0.14 : 0.32}
      ...
      {scenario.nodes.map((n) => {
        ...
        const stroke = isCore ? color : n.kind === 'runtime' ? '#64748b' : n.kind === 'user' || n.kind === 'human' ? '#94a3b8' : color
